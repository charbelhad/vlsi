##########################################################################################
# DFT configuration for Design Compiler methodology scripts
# Script: scan_configuration.dc.tcl
# Version: U-2022.12
...
# Configuring DFT with Internal pins flow
# Note: most of the following flows (with the exception of THIRDPARTY-CODEC_COREWRAP-MM_TP_SCAN) utilize 
#  top level ports as DFT signals. These ports should either already exist in your RTL, or have been created
#  using the dft_ports.tcl script.
# If there are any DFT signals that originate from internal pin, you must enable the internal pins flow. 
# Refer to the example in the THIRDPARTY-CODEC_COREWRAP-MM_TP_SCAN section for details

puts "RM-info: Configuring for DFT_CONFIGURATION = $DFT_CONFIGURATION"

switch $DFT_CONFIGURATION {

  DC-CODEC_COREWRAP_TP_SCAN {
    ...
    set_dft_signal -type wrp_shift -port snps_core_wrap_shift_en -view spec -test_mode {snps_wrp_of}
    # Specify the following if you have separate input and output wrapper shift enable, 
    #   then use set_wrapper_configuration -input_shift_enable and -output_shift_enable option
    ...
    # Enable Codec insertion
    # If you have selected DFTMAX Compression, then enable the following command
    ...
    # For DFTMAX Ultra, 
    # comment out "set_dft_configuration -scan_compression enable" command above, 
    # uncomment  "set_dft_configuration -streaming_compression enable" command below,
    # then use set_streaming_compression_configuration command below to specify codec parameters and specify clock signal for codec
    # Use DFT_INTERNAL_SCAN_CHAIN_COUNT and DFT_COMPRESSION_SCAN_CHAIN_COUNT from variables.tcl

    # If you have selected DFTMAX Ultra Compression, comment out the command above and uncomment the following
    ...
    set_wrapper_configuration -class core_wrapper -mix_cells true 
    # Assume core wrapper chain counts needed for snps_wrp_if and snps_wrp_of modes
    set_wrapper_configuration -class core_wrapper -chain_count ${DFT_WRAPPER_CHAIN_COUNT} -test_mode snps_wrp_if
    set_wrapper_configuration -class core_wrapper -chain_count ${DFT_WRAPPER_CHAIN_COUNT} -test_mode snps_wrp_of
    ...
    set_scan_configuration -chain_count ${DFT_INTERNAL_SCAN_CHAIN_COUNT} -test_mode snps_scan
    set_scan_configuration -chain_count ${DFT_INTERNAL_SCAN_CHAIN_COUNT} -test_mode snps_wrp_if
  ...
  THIRDPARTY-CODEC_COREWRAP-MM_TP_SCAN {
    ...
    #  # set_wrapper_configuration -mix_cells true -test_mode {}
    ...
    #  # Configuring Scan
    #  # Control mixing of clocks and power domains for wrapper chains. 
    #  set_scan_configuration -clock_mixing true -type_mode wrp_of
    #  set_scan_configuration -power_mixing true -type_mode wrp_of

    #  # Chain counts for scan modes
    #  set_scan_configuration -chain_count ${INTERNAL_SCAN_CHAIN_COUNT} -test_mode snps_scan
    #  set_scan_configuration -chain_count ${INTERNAL_SCAN_CHAIN_COUNT} -test_mode snps_wrp_if
