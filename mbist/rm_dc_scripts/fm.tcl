...
# design, then the hierarchical designs will be removed and power model 
# will be read in and use 

switch $RTL_SOURCE_FORMAT {
  autoread {
    # Source the script generated by autoread in Design Compiler
    rm_source -file ${OUTPUTS_DIR}/${DESIGN_NAME}.autoread_rtl.tcl
  }
  script {
    # The following setting allows the read script to be found in the search path
    set_app_var sh_source_uses_search_path true
    rm_source -file ${FMRM_RTL_READ_SCRIPT} -print FMRM_RTL_READ_SCRIPT
  }
  vhdl {
...
if {$DESIGN_STYLE == "hier" && $PHYSICAL_HIERARCHY_LEVEL == "top"} {
  ...
  # Remove the hierarchical designs for a top-only verification
  foreach design ${HIER_DESIGNS} {
    remove_design r:/WORK/${design}
  }
  
  # Read power models for the reference container
  foreach design ${HIER_DESIGNS} {
    read_power_model -r ${design}.ref.fpm -libname WORK
  }
}
...
# For DDC
#read_ddc -i ${OUTPUTS_DIR}/${DESIGN_NAME}.mapped.ddc

# OR

# For Verilog
read_verilog -i ${OUTPUTS_DIR}/${DESIGN_NAME}.mapped.v

if {$DESIGN_STYLE == "hier" && $PHYSICAL_HIERARCHY_LEVEL == "top"} {
  # Read power models for the Implementation container
  foreach design ${HIER_DESIGNS} {
    read_power_model -i ${design}.impl.fpm
  }
}
...
if {$UPF_MODE != "none"} {
  set load_upf_cmd "load_upf -i"
  if {$UPF_MODE == "golden"} {
    lappend load_upf_cmd -strict_check false
    lappend load_upf_cmd -target dc_netlist ${UPF_FILE}
    lappend load_upf_cmd -supplemental ${OUTPUTS_DIR}/${DESIGN_NAME}.supplement.upf
  } elseif { $UPF_MODE == "prime" } {
    lappend load_upf_cmd ${OUTPUTS_DIR}/${DESIGN_NAME}.mapped.upf
  }
...
preverify
match
save_session -replace ${OUTPUTS_DIR}/${DESIGN_NAME}_match

redirect -file ${REPORTS_DIR}/${DESIGN_NAME}.fmv_unmatched_points.rpt {report_unmatched_points}
...
if { ![verify] }  {  
  redirect -file ${REPORTS_DIR}/${DESIGN_NAME}.fmv_failing_points.rpt {report_failing_points}
  redirect -file ${REPORTS_DIR}/${DESIGN_NAME}.fmv_aborted_points.rpt {report_aborted}
  ...
  redirect -file ${REPORTS_DIR}/${DESIGN_NAME}.fmv_analyze_points.rpt {analyze_points -all}
}

save_session -replace ${OUTPUTS_DIR}/${DESIGN_NAME}_verify

if {$DESIGN_STYLE == "hier" && $PHYSICAL_HIERARCHY_LEVEL == "bottom"} {
  write_power_model -r -replace ${OUTPUTS_DIR}/${DESIGN_NAME}.ref.fpm
  write_power_model -i -replace ${OUTPUTS_DIR}/${DESIGN_NAME}.impl.fpm
}
